...
class BCInterface:
    ...
    def __init__(self,host='localhost',port=5001,mainnet=False):
        ...
        if mainnet:
            ...
            ipc_path = os.path.dirname(os.path.realpath(__file__))+'/data_privnet/geth.ipc'
    ...
    def new_account(self):
        while True:
            pwd1 = getpass('Enter a password: ')
            pwd2 = getpass('Repeat your password: ')
            if pwd1 != pwd2:
                print('Passwords do not match!')
            if pwd1 == pwd2:
                print('Creating new Ethereum account...')
                self.web3.personal.newAccount(pwd1)
                break
